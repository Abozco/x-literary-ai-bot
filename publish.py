...
def post():
    try:
        ...
        print("تم نشر التغريدة/الثريد بنجاح!")
    except Exception as e:
        print("خطأ عند النشر:", e)

def reply_mentions():
    try:
        me_data = client.get_me().data
        if not me_data:
            print("خطأ: لم يتم الحصول على معرف المستخدم، تحقق من التوكنات.")
            return
        
        me = me_data.id
        print("معرف المستخدم:", me)

        mentions = client.get_users_mentions(id=me, max_results=3)
        if not mentions.data:
            print("لا توجد إشارات جديدة للرد عليها.")
            return
        ...
        print("تم الرد على الإشارات بنجاح!")
    except Exception as e:
        print("خطأ عند الرد على الإشارات:", e)
